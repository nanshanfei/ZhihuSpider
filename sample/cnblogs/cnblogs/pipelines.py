...
class MySQLStoreCnblogsPipeline(object):
    ...
    def _do_upinsert(self, conn, item, spider):
        ...
        if ret:
            conn.execute("""
                update cnblogsinfo set page_url = %s, title = %s, description = %s, link = %s, listUrl = %s, updated = %s where linkmd5id = %s
            """, (item['page_url'], item['title'], item['desc'], item['link'], item['listUrl'], now, linkmd5id))
            #print """
            ...
            conn.execute("""
                insert into cnblogsinfo(linkmd5id, page_url, title, description, link, listUrl, updated)
                values(%s, %s, %s, %s, %s, %s, %s)
            """, (linkmd5id, item['page_url'], item['title'], item['desc'], item['link'], item['listUrl'], now))
            #print """
